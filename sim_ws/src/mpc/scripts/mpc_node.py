...
class mpc_config:
    ...
    MIN_STEER: float = -0.4  # maximum steering angle [rad]
...
class MPC(Node):
    ...
    def __init__(self):
        ...
        self.pose_sub_ = self.create_subscription(Odometry, 'ego_racecar/odom', self.pose_callback, 10)

        self.pose_sub_  # prevent unused variable warning
        
        self.drive_pub = self.create_publisher(AckermannDriveStamped,'/drive',10) 
        
        self.drive_pub  # prevent unused variable warning
        ...
        self.v_factor = 5. #adjust the ref speed

        #       use the MPC as a tracker (similar to pure pursuit)
        # TODO: get waypoints here
        # csv_file_path = '/home/tim/rcws/logs/wp-2024-04-09-03-28-09.csv' # slam map
        # waypoint_list = []
        ...
        data = np.genfromtxt('/home/tim/wp-2024-04-09-03-28-09.csv', delimiter=',')
        ...
        self.v_values = data[:, 2]
        self.yaw_values = data[:, 3]


        self.waypoints = np.column_stack((self.x_values, self.y_values, self.v_values, self.yaw_values))
        ...
        self.x_values = self.waypoints[:, 0]  # Selecting all rows from the first column
        self.y_values = self.waypoints[:, 1]  # Selecting all rows from the second column
        self.v_values = self.waypoints[:, 2] * self.v_factor
        self.yaw_values = self.waypoints[:, 3]
    ...
    def pose_callback(self, pose_msg):
        ...
        ref_path = self.calc_ref_trajectory(vehicle_state, self.x_values, self.y_values, self.v_values, self.yaw_values)
        x0 = [vehicle_state.x, vehicle_state.y, vehicle_state.v, vehicle_state.yaw]

        # TODO: solve the MPC control problem
        (self.oa,self.odelta,_,_,_,_,_,) = self.linear_mpc_control(ref_path, x0, self.oa, self.odelta)
        print(self.odelta)

        # TODO: publish drive message.
        steer_ang = self.odelta[0]
    ...
    def mpc_prob_init(self):
        ...
        final_state_dev = self.xk[:, -1] - self.ref_traj_k[:, -1]
        print(final_state_dev)

        objective += cvxpy.quad_form(final_state_dev, self.config.Qfk)
